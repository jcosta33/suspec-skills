...
The catalog's Suspec authoring guides ([`write-spec`](../skills/write-spec/SKILL.md), [`write-audit`](../skills/write-audit/SKILL.md), [`write-research`](../skills/write-research/SKILL.md), [`write-bug-report`](../skills/write-bug-report/SKILL.md), …) sit in the same authorial band, but in the Suspec workflow their working state lives in the task packet itself, so they ship no separate template. Beyond `fix-flaky-test` (above), no catalog discipline ships a bundled session frame — the rest carry their working structure inline in the `SKILL.md` body.
