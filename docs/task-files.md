...
| **Where it lives** | A gitignored local folder (`.tasks/<slug>.md` by convention)                                      | Next to your own native artifacts, agent-chosen spot — out of the repo by default unless the project's own governance says otherwise (a code change is the one exception: it lands in the source tree by definition) |
| **Lifetime**       | Until the deliverable lands; then discarded (no archival value)                                   | As long as the project itself                                                                                                                                              |
| **Visible in PRs** | No                                                                                                | No by default (Yes if the project's governance puts it in-repo; a code change always is)                                                                                  |
| **Per-machine**    | Yes                                                                                               | Yes by default (No if committed by governance; a code change is always shared via git)                                                                                    |

If the task file were committed instead, the repo would accumulate thousands of stale scratchpads within weeks, diluting what actually belongs in git history — the code changes, or any docs a project's own governance chooses to commit. The split is structural: the task file is the _workspace_, the deliverable is the _output_.
...
This catalog's code-authoring skills (the `write-*` family) each ship a `references/task-template.md`, and `fix-flaky-test` an equivalent `references/notes.md` — each scoring ≥3 on the MIHPSG rubric. The shape of the score predicts the shape of the template:
...
The catalog's Suspec authoring guides ([`write-spec`](../skills/write-spec/SKILL.md), [`write-audit`](../skills/write-audit/SKILL.md), [`write-research`](../skills/write-research/SKILL.md), [`write-bug-report`](../skills/write-bug-report/SKILL.md), …) sit in the same authorial band, but their working state lives in the task packet when one is cut, or the spec's own `## Execution` section for 1:1 work (most work), so they ship no separate template. Beyond `fix-flaky-test` (above), no catalog discipline ships a bundled session frame — the rest carry their working structure inline in the `SKILL.md` body.
...
| `implement-task`       | For 1:1 work (no task) the spec's own `## Execution` section **is** the working state; for a split task, the task packet **is** — either way the guide fills the artifact's own sections rather than shadowing it with a second file. If the deliverable and the working state are the same document, ship none. |
...
| **Iterative skills carry a hypothesis trail**                 | `write-fix` ships `## Hypothesis trail`; [`fix-flaky-test`](../skills/fix-flaky-test/references/notes.md)'s `## Hypothesis tracker` records what each rejection teaches — both per Reflexion [\[27\]](./sources.md#27)'s verbal-feedback loop                                                                                                                                                                                                                |
