...
status, and decisive raw output. Keep short output once. Move dominating output to an adjacent
receipt with stable anchors, then map each claim to its anchor. Never paraphrase proof or reuse stale
output after a relevant edit.

## Handoff

When the artifact is the deliverable, return its absolute clickable path without repeating its
contents. Explain only blockers, failed creation, incomplete verification, or irreversible
confirmation.
