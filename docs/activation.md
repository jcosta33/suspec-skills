...
The 800-char line is this repo's forcing target, not the spec limit (1024 [\[1\]](./sources.md#1)). Most shipped descriptions sit under it; two exceptions run past it — [`revolver-review`](../skills/revolver-review/SKILL.md) and [`bulletproof`](../skills/bulletproof/SKILL.md) — each spec-legal, each carrying a full posture plus a procedure in its trigger. They are explicit exceptions, not silent ones; a third would be a smell.
