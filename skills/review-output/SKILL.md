...
the store directory your dispatch prompt points into — no dispatch prompt? Resolve the store
first: `suspec store path` prints the exact directory (collision-safe); without the CLI,
default to `~/.claude/state/<repo-name>/`. It proves one thing: **the code honors the spec** — every
